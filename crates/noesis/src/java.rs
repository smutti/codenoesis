...
use codenoesis_application::{
    PublicationService, ScanError, ScanRequest, ScanService, s8_java::JavaScanError,
};
use codenoesis_contracts::{CodeNoesisErrorV4, CodeNoesisErrorV6, s8_java as contracts};
use codenoesis_domain::{
    RepositoryIdentity, Revision,
    s8_java::{JavaError, MAX_OUTPUT_BYTES, PROFILE, SNAPSHOT_VERSION},
    storage::{ArtifactRole, LocalSnapshotHead},
};
use codenoesis_lang_java::workspace::TreeSitterJavaWorkspaceExtractor;
...
impl Failure {
    fn new(code: &str, stage: &str, exit: u8) -> Self {
        Self {
            value: json!({"schema_version":"codenoesis.java-error/v1","code":code,"stage":stage,"message":code,"retryable":false,"context":{}}),
            exit,
        }
    }
    fn input() -> Self {
        Self::new("input.invalid_java_arguments", "input", 2)
    }
    fn internal() -> Self {
        Self::new("internal.java_failure", "internal", 1)
    }
    fn artifact() -> Self {
        Self::new(
            "artifact.invalid_or_unsafe_java_projection",
            "projection",
            16,
        )
    }
    fn extraction(error: JavaError) -> Self {
        let code = match error {
            JavaError::InvalidUtf8 => "extraction.java_invalid_utf8",
            JavaError::SourceBoundary(_) => "extraction.java_source_boundary",
            JavaError::InvalidSyntax => "extraction.java_invalid_syntax",
            JavaError::NoJavaSources => "extraction.no_java_sources",
            JavaError::InvalidContract => "contract.invalid_java_graph",
            JavaError::LimitExceeded(_) => "limit.java_capacity_exceeded",
        };
        let mut failure = Self::new(code, "extraction", 11);
        if let JavaError::LimitExceeded(limit) = error {
    ...
    fn scan(error: ScanError) -> Self {
        ...
        value["schema_version"] = json!("codenoesis.java-error/v1");
        Self { value, exit: 10 }
    }
}
...
pub(crate) fn requested(arguments: &[OsString]) -> bool {
    arguments.iter().any(|a| {
        a == "--java-profile" || a.to_str().is_some_and(|s| s.starts_with("--java-profile="))
    })
}
pub(crate) fn security_failure() -> ExitCode {
    emit(&Failure::new(
        "security.java_boundary_unavailable",
        "security",
        1,
    ))
}
...
fn emit(failure: &Failure) -> ExitCode {
    let bytes=contracts::canonical(&failure.value).unwrap_or_else(|_|b"{\"schema_version\":\"codenoesis.java-error/v1\",\"code\":\"internal.serialization\"}\n".to_vec());
...
impl Invocation {
    fn parse(arguments: &[OsString]) -> Result<Self, Failure> {
        ...
        let allowed: &[&str] = match command {
            ...
            "docs" => &["--java-profile", "--store", "--repository-identity"],
            "explore" => &["--java-profile", "--input", "--output"],
        ...
        if options.get("--java-profile").and_then(|v| v.to_str()) != Some(PROFILE) {
            return Err(Failure::input());
        }
        let invocation = Self {
...
fn scan(invocation: &Invocation, worker: &mut ScanWorker) -> Result<Vec<u8>, Failure> {
    ...
    let snapshot = run_confined_scan(worker, repository.clone(), None, Vec::new(), move || {
        ...
        ScanService::new(LocalGitRepository::new_packed_sha1())
            .scan_java(
                ...
                &TreeSitterJavaWorkspaceExtractor,
            )
            .map_err(|e| match e {
                JavaScanError::Scan(e) => Failure::scan(e),
                JavaScanError::Extraction(e) => Failure::extraction(e),
            })
    })
    .map_err(|()| Failure::internal())??;
    if started.elapsed().as_millis() > 60_000 {
        return Err(Failure::extraction(JavaError::LimitExceeded(
    ...
    let mut local = LocalStore::open(Path::new(&repository), Path::new(&store))
        .map_err(|e| Failure::scan(ScanError::Storage(e)))?;
    PublicationService::publish_java(
        &snapshot,
        &mut local.artifacts,
        &mut local.metadata,
...
fn project(invocation: &Invocation) -> Result<Vec<u8>, Failure> {
    ...
    if invocation.command == "export" {
        ...
        let prepared =
            noesis::portable_explorer::java::prepare(Path::new(store), Path::new(output), false)
                .map_err(|_| Failure::artifact())?;
        noesis::install_r8_export_filesystem_boundary(store, output)
            .map_err(|_| Failure::internal())?;
        return noesis::portable_explorer::java::publish(&prepared, &bytes, false)
            .map_err(|_| Failure::artifact());
    }
    ...
    let matches: Vec<_> = graph["entities"]
        ...
        .collect();
    contracts::canonical(&json!({"schema_version":"codenoesis.java-query/v1","snapshot_id":head.snapshot_id.as_str(),"total_matches":matches.len(),"truncated":matches.len()>100,"results":matches.into_iter().take(100).collect::<Vec<_>>()})).map_err(Failure::extraction)
}
fn explore(invocation: &Invocation) -> Result<Vec<u8>, Failure> {
    let input = noesis::portable_explorer::java::input_path(Path::new(invocation.get("--input")?))
        .map_err(|_| Failure::artifact())?;
    ...
    let output = invocation.get("--output")?;
    let prepared = noesis::portable_explorer::java::prepare(&input, Path::new(output), true)
        .map_err(|_| Failure::artifact())?;
    noesis::install_r8_explorer_filesystem_boundary(input.as_os_str(), output)
        .map_err(|_| Failure::internal())?;
    noesis::portable_explorer::java::publish(&prepared, &bytes, true)
        .map_err(|_| Failure::artifact())
}
