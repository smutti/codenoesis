//! Java projections use the existing marker-owned publication protocol.
use super::{OutputKind, OwnedFile, PortableExplorerError, PreparedOutputRoot};
use codenoesis_contracts::s8_java::{canonical, parse_portable};
use serde_json::json;
...
pub fn prepare(
    ...
    super::ensure_output_root(
        ...
        output,
        if explorer {
            OutputKind::JavaExplorer
        } else {
            OutputKind::JavaPortable
        },
    )
}
...
pub fn publish(
    ...
    let payload = serde_json::to_string(&portable["payload"])
        ...
        .replace('<', "\\u003c")
        .replace('>', "\\u003e");
    let html = viewer_html(include_bytes!("../../assets/s8/java/index.html"), &payload)?;
    let manifest = json!({"schema_version":"codenoesis.java-explorer/v1","snapshot_id":portable["payload"]["snapshot_id"],"files":[
...
fn viewer_html(template: &[u8], payload: &str) -> Result<Vec<u8>, PortableExplorerError> {
    ...
    Ok(template.replace("__JAVA_PAYLOAD__", payload).into_bytes())
}
...
mod tests {
    ...
    #[test]
    fn pt_fr_ext_026_viewer_bytes_are_identical_for_lf_and_crlf_checkouts() {
        let checked_out = include_bytes!("../../assets/s8/java/index.html");
