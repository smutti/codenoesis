#!/usr/bin/env python3
"""Measure pinned Java repositories; retain every attempt, including failures."""
import argparse
...
def main():
    ...
    manifest = json.loads((root / "tests/specifications/s8/java-public-v1.json").read_text())
    binary = args.binary.resolve(strict=True)
    corpus = args.corpus.resolve(strict=True)
    output = args.output.resolve()
    output.mkdir(parents=True, exist_ok=False)
    report = {"schema_version": "codenoesis.java-benchmark-observation/v1",
              ...
              "environment": {"os": platform.platform(), "architecture": platform.machine(), "rustc": subprocess.check_output(["rustc", "--version"], text=True).strip()},
    ...
    for repository in manifest["repositories"]:
        ...
        for repeat in range(1, manifest["repetitions"] + 1):
            ...
            command = [str(binary), "scan", "--profile", "standard-local-s8", "--java-profile",
                       "java-declarations-v1", "--repository", str(corpus / repository["name"]),
                       "--revision", repository["revision"], "--repository-identity",
                       f"urn:codenoesis:repository:java-{repository['name']}", "--store", str(output / f"{prefix}-store")]
            ...
            if exit_code == 0:
                ...
                sample.update(semantic_hash=snapshot["semantic_hash"]["value"], entities=len(graph["entities"]),
                              declarations=sum("owner" in e["properties"] for e in graph["entities"]),
                              ...
                              syntax_gap_paths=[gap["path"] for gap in graph["coverage"] if gap["reason"] == "java_syntax_not_accepted_by_pinned_parser"])
            else:
                try:
