...
> Status: **0.9+r14 — S0 through S6 and R0-R13 plus K1 are implemented but not
> Verified; one additive R14 expression/binding layer is proposed**.
> Protected merge #161 made the R13 V12/V15 family effective. Decision 0025
> proposes a committed-source expression and lexical-binding layer in V13/V16
> while preserving every R0-R13/K1 selector, contract, identity, golden, and
> viewer byte.
...
| Version | `0.9` |
| Status | S0 through S6 and roadmap R0-R13 plus K1 are Approved and Implemented but not Verified. `FR-EXT-016`, `FR-EXP-006`, and bounded R14 amendments are Proposed and become Approved and Implemented only after the accountable maintainer manually merges the exact protected pull request for issue #162. |
| Date | 2026-08-10 |
...
### 2.25 S4 R14 Rust expression and lexical-binding candidate register
...
and [Decision 0025](decisions/0025-s4-r14-rust-expression-bindings-contract.md)
govern one high-risk maintainer-supervised package on exact base
`e32428ecac33df384b2e8b6eed3d257da06e18fe`. Governance, retained Red,
candidate code, tests, documentation, and evidence become effective atomically
only after protected manual merge. Until then every R14 amendment and
implementation statement remains Proposed/candidate.
...
| Requirement | Current state | Target after protected merge | Slice | Ratification material |
|---|---|---|---|---|
| `FR-EXT-016` | Proposed | `Approved` only for the closed `rust-expression-bindings-v1` syntax and lexical-binding profile | `S4` | Decision 0025, [closed profile](../../tests/specifications/s4/r14/expression-bindings-v1.json), [machine oracle](../../tests/specifications/s4/r14/e2e_fr_ext_016_rust_expression_bindings.json), and [fixture descriptor](../../tests/fixtures/s4/rust-expression-bindings-v1/README.md) |
| `FR-EXP-006` | Proposed | `Approved` only for lossless V16 `PortableGraphV7` and read-only `LocalExplorerV7` | `S4` | V7 schemas, Decision 0025, and strict reimport oracle |
| Bounded R14 amendment to `FR-EXT-012` | K1 Approved and Implemented | `Approved` for preserving complete K1 while adding only committed-source expression, pattern-binding, and lexical-access occurrence facts | `S4` | Decision 0025 and exact additive oracle |
| Bounded R14 amendments to `FR-DOC-001/002/003` | Prior S4 docs Approved | `Approved` for evidence-backed expression/binding sections explicitly labelled syntax-only | `S4` | Decision 0025 and E2E journey |
| Bounded R14 amendment to `FR-QRY-001` | LocalQueryResultV1-V10 Approved | `Approved` with additive LocalQueryResultV11 for expression, argument, binding, and access subjects | `S4` | [LocalQueryResultV11 schema](../../tests/specifications/s4/r14/local-query-result-v11.schema.json) |
| Bounded R14 amendment to `FR-CLI-001` | Prior local journeys Approved | `Approved` for the explicit K1+R14 scan/docs/query/export/explore journey and ErrorV21 after complete intent | `S4` | Decision 0025, selector dispatch, and machine oracle |
| Bounded R14 amendments to `NFR-DET-001`, `NFR-SEC-001/005`, and `NFR-PRV-002` | Existing local controls Approved | `Approved` for 50 permutations, ten schedules, no execution/network, immutable K1 input, and private V7 projection | `S4` | Determinism, security, privacy, and race oracle |
| Bounded R14 amendments to `NFR-TST-001/002` | Red-first deterministic evidence required | `Approved` with committed governance checkpoint, retained expected Red, focused/full Green, and immutable regressions | `S4` | R14 evidence pack |
...
| ID | Pri. | Target | Normative requirement | Acceptance evidence |
|---|---:|---:|---|---|
| `FR-DOC-001` | P0 | `0.1` | The local product MUST generate deterministic overview and module documentation from validated claims and evidence. GraphV9 documentation MUST expose declaration alternatives without choosing an active configuration; GraphV11 MUST expose occurrence-specific callable facts; GraphV12 MUST expose compiler correspondence as a validated artifact assertion; GraphV13 MUST expose expressions, bindings, and lexical access explicitly as syntax-only facts distinct from data flow and runtime behavior. | Repeated generation is byte-identical apart from explicitly non-canonical envelope data; every R10/R12 alternative, R13 callable/compiler join, and R14 expression/binding/access fact is discoverable with explicit epistemic limits. `GT`, `E2E` |
| `FR-DOC-002` | P0 | `0.1` | Every material generated statement MUST resolve to valid evidence or be rendered as unknown, contradictory, or unsupported. R10/R12 alternative statements, R13 callable/compiler correspondence, and R14 expression/binding/access statements MUST resolve to exact evidence and preserve their syntax-only epistemic boundary. | Deleting or corrupting evidence blocks the claim or changes its explicit state; no unsupported prose, inferred active `cfg`, compiler generation, data flow, call, ownership, side effect, or runtime meaning remains. `PT`, `E2E` |
| `FR-DOC-003` | P0 | `0.1` | Generated output MUST be confined to a configured generated-document location and MUST NOT overwrite hand-written documentation. The same marker-owned atomic rule applies to R10-R14 docs, portable, and explorer roots. | Checksums of manual files remain unchanged after generation and failure recovery; path/symlink/race cases fail without partial R10-R14 output. `E2E`, `SEC` |
...
| `FR-QRY-001` | P0 | `0.1` | Local queries MUST retrieve every approved entity, relationship, claim, evidence, diagnostic, coverage gap, and document by stable identity, expose unknown or contradictory states, and select a versioned result contract from the validated snapshot without changing prior result versions. R8 MAY expose the same exact subjects in a validated read-only portable projection. K1 adds LocalQueryResultV6 for V11; R10 adds V7 for V12; R11 adds V8 for V13; R12 adds V9 for V14; R13 adds V10 for V15; and R14 adds LocalQueryResultV11 for V16 expression, argument, binding, and access subjects, while V1-V10 bytes and authority remain unchanged. | CLI black-box scenarios return the reviewed typed result and stable exit status; V16 exercises every R14 subject after restart, V15 exercises R13, V14 R12, V13 R11, V12 R10, V11 K1, earlier heads retain approved dispatch/bytes, and portable family digests prove exact preservation. `E2E`, `CONF` |
...
| `FR-CLI-001` | P0 | `0.1` | The CLI MUST provide local `scan`, `docs`, `query`, `export`, and `explore` journeys with human-readable and versioned JSON output. R8, K1, and R10-R14 behavior MUST be explicitly selected, preserve accepted command bytes, use marker-owned atomic output roots, and reject unsupported profile composition before acquisition. | The R8 through R14 fixtures each complete scan -> docs -> query -> export -> explore without network, child process, browser auto-launch, target/toolchain/index execution, repository mutation, or unintended store mutation; complete invalid R14 composition returns ErrorV21, incomplete selector intent retains its historical error family, and every prior command remains unchanged. `E2E`, `CONF`, `SEC`, `FT` |
