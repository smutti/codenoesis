...
### 2.33 G1a/S14 local CLI distribution and configuration candidate register
...
govern one branch-scoped candidate on exact protected base
`a525126228205901885038586e21d30db745b1ec`. Governance, strict schemas,
configuration and bundle fixtures, retained configuration and packaging Reds,
minimum implementation, tests, procedures, traceability, and evidence share
one supervised pull request. Before protected manual merge, every G1a
requirement and implementation statement remains Proposed/candidate.

| Requirement | Current state | Candidate after protected merge | Slice | Ratification material |
|---|---|---|---|---|
| `FR-CFG-001` | Proposed, branch-scoped implementation candidate | Approved and Implemented only for closed `LocalCliConfigurationV1`, embedded-default/explicit-file precedence, stable explicit-file validation, canonical `LocalConfigurationReportV1`, and strict `CodeNoesisErrorV26`; not Verified | `S14` | Issue #182, Decision 0034, configuration schemas/goldens, retained Red/Green, and complete gate |
| `FR-REL-002` | Proposed, branch-scoped implementation candidate | Approved and Implemented only for one deterministic unsigned digest-named staged-directory bundle and explicit clean-install, side-by-side upgrade, rollback, and uninstall procedures; not Verified, signed, supported, published, or GA | `S14` | Issue #182, Decision 0034, manifest/tree oracle, lifecycle fixture, retained Red/Green, and complete gate |
| `FR-CLI-008` | Proposed, branch-scoped implementation candidate | Approved and Implemented only for `noesis config validate`, the optional leading `--config` pair, and `xtask package-local-cli` with exact streams, exits, limits, and failure atomicity; not Verified | `S14` | Issue #182, Decision 0034, two public E2E oracles, privacy/boundary tests, and complete gate |
...
Protected merge would activate no server artifact, archive or installer,
