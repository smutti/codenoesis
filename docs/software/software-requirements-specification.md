...
> Status: **0.9+r14.1 — S0 through S6 and R0-R14 plus K1 are implemented but
> not Verified; one bounded empty-R5 neutrality correction is a branch-authorized
> candidate**. Protected merge #163 made the R14 V13/V16 family effective.
> Decision 0026 corrects only the invalid additive non-emptiness invariant while
> preserving every R0-R14/K1 selector, contract, identity, golden, and viewer
> byte.
...
| Version | `0.9` |
| Status | S0 through S6 and roadmap R0-R14 plus K1 are Approved and Implemented but not Verified. Issue #164 is an authorized high-risk S4 correction candidate for approved `FR-EXT-010`; it becomes effective only after protected manual merge. |
| Date | 2026-08-11 |
...
Before production code, the checkpoint E2E must retain the reviewed ErrorV21
...
The correction becomes effective only after protected manual merge and remains
not Verified until its complete evidence is independently accepted.
