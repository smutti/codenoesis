...
| `S8` Polyglot adapters | Start with bounded Kotlin/KMP (Decision 0051 candidate); add Java, JavaScript/TypeScript, and C/C++ one adapter at a time. | `FR-EXT-003/005/025` | Run the shared semantic capability fixture for each adapter. | Contract, golden, malformed, determinism, and differential suites green per language. |
