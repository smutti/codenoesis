...
| `FR-EXT-025` | P1 | `0.1` candidate | The S8 Kotlin/KMP profile MUST extract bounded committed declarations, conventional module/source-set membership and evidence-backed Unknown expect/actual candidates under [Decision 0051](decisions/0051-s8-kotlin-kmp-declarations.md), publish a v19 local snapshot and provide query, documentation, portable export and offline exploration. It MUST NOT execute Gradle, resolve effective source sets/types, infer actualization or change existing Rust/S7 contracts. Unsupported syntax and roots MUST remain explicit gaps; invalid encoding, acquisition, capacity, integrity and unsafe publication inputs MUST fail with typed errors. | Hand-authored declaration oracle, malformed/Unicode/parser-limit tests, immutable scan/store/query/export/viewer journey, ambiguity/decoy and artifact security checks, plus three retained observations on each of two pinned public repositories. `CT`, `GT`, `SEC`, `E2E`, `CONF` |
| `FR-EXT-026` | P1 | `0.1` candidate | The S8 Java profile MUST extract committed package/import/type/member declarations, distinct overloads and field declarators, named nesting and conventional Maven/Gradle module/source-set membership under [Decision 0052](decisions/0052-s8-java-declarations.md). It MUST publish a v20 snapshot and query/docs/export/offline views with exact source evidence. Unicode preprocessing, JPMS, unsupported syntax and nonconventional roots MUST remain explicit gaps; encoding, acquisition, capacity, integrity and unsafe publication inputs MUST fail with typed errors. Build execution, classpath/type resolution, generated members and runtime behavior are excluded. | Hand-authored 26-declaration/22-containment oracle, parser and portable contract tests, immutable scan/store/query/docs/export/viewer journey, security failures, plus three retained observations per pinned public sample. `CT`, `GT`, `SEC`, `E2E`, `CONF` |
...
| `S8` Polyglot adapters | Bounded Kotlin/KMP (Decision 0051) and Java declarations (Decision 0052 candidate); add remaining adapters separately. | `FR-EXT-003/005/025/026` | Run the shared semantic capability fixture for each adapter. | Contract, golden, malformed, determinism, and differential suites green per language. |
...
### Kotlin/KMP candidate status

FR-EXT-025 is Proposed on this branch under Decision 0051 and the maintainer's
authorization to proceed with the first P1/S8 vertical. Its bounded semantics
and implementation become effective together after manual merge. This does
not broaden FR-EXT-003 or claim that the remaining S8 adapters are implemented.

### Java candidate status

FR-EXT-026 is Proposed on this branch under Decision 0052 and the maintainer's authorization to proceed after Kotlin PR #225. Its exact semantics become effective after manual merge. It does not complete S8, provide compiler semantics or broaden the frozen Rust contract.
