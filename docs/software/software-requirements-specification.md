...
> Status: **0.9+r13 — S0 through S6 and R0-R12 plus K1 are implemented but not
> Verified; one additive R13 callable/SCIP composition is proposed**.
> Protected merge #159 made the R12 V11/V14 family effective. Decision 0024
> proposes an evidence-preserving K1/R7 join in V12/V15 while preserving every
> R0-R12/K1 selector, contract, identity, golden, and viewer byte.
...
| Version | `0.9` |
| Status | S0 through S6 and roadmap R0-R12 plus K1 are Approved and Implemented but not Verified. `FR-EXT-015`, `FR-EXP-005`, and bounded R13 amendments are Proposed and become Approved and Implemented only after the accountable maintainer manually merges the exact protected pull request for issue #160. |
| Date | 2026-08-10 |
...
### 2.24 S4 R13 K1 and revision-bound SCIP composition candidate register
...
govern one maintainer-supervised package on exact base
`cc25dec49343c510f124585c91d459982e827c68`. Governance, retained Red,
candidate code, tests, documentation, and evidence become effective atomically
only after protected manual merge. Until then every R13 amendment and
implementation statement remains Proposed/candidate.
...
| Requirement | Current state | Target after protected merge | Slice | Ratification material |
|---|---|---|---|---|
| `FR-EXT-015` | Proposed | `Approved` only for the exact evidence-backed K1/R7 `HAS_COMPILER_SYMBOL` composition | `S4` | Decision 0024, [composition contract](../../tests/specifications/s4/r13/callable-scip-composition-v1.json), [machine oracle](../../tests/specifications/s4/r13/e2e_fr_ext_015_k1_scip_composition.json), and [fixture descriptor](../../tests/fixtures/s4/rust-callable-scip-composition-v1/README.md) |
| `FR-EXP-005` | Proposed | `Approved` only for lossless V15 `PortableGraphV6` and read-only `LocalExplorerV6` | `S4` | V6 schemas, Decision 0024, and strict reimport oracle |
...
| ID | Pri. | Target | Normative requirement | Acceptance evidence |
|---|---:|---:|---|---|
| `FR-DOC-001` | P0 | `0.1` | The local product MUST generate deterministic overview and module documentation from validated claims and evidence. GraphV9 documentation MUST expose declaration alternatives without choosing an active configuration; GraphV11 MUST expose occurrence-specific callable facts; GraphV12 MUST expose compiler correspondence as a validated artifact assertion distinct from source and runtime facts. | Repeated generation is byte-identical apart from explicitly non-canonical envelope data; every R10/R12 alternative and every R13 callable/compiler join is discoverable with explicit uncertainty. `GT`, `E2E` |
| `FR-DOC-002` | P0 | `0.1` | Every material generated statement MUST resolve to valid evidence or be rendered as unknown, contradictory, or unsupported. R10/R12 alternative statements and R13 callable/compiler correspondence MUST resolve to their exact source and artifact evidence as applicable. | Deleting or corrupting evidence blocks the claim or changes its explicit state; no unsupported prose, inferred active `cfg`, compiler generation, call, or runtime meaning remains. `PT`, `E2E` |
| `FR-DOC-003` | P0 | `0.1` | Generated output MUST be confined to a configured generated-document location and MUST NOT overwrite hand-written documentation. The same marker-owned atomic rule applies to R10-R13 docs, portable, and explorer roots. | Checksums of manual files remain unchanged after generation and failure recovery; path/symlink/race cases fail without partial R10-R13 output. `E2E`, `SEC` |
...
| `FR-QRY-001` | P0 | `0.1` | Local queries MUST retrieve every approved entity, relationship, claim, evidence, diagnostic, coverage gap, and document by stable identity, expose unknown or contradictory states, and select a versioned result contract from the validated snapshot without changing prior result versions. R8 MAY expose the same exact subjects in a validated read-only portable projection. K1 adds LocalQueryResultV6 for V11; R10 adds V7 for V12; R11 adds V8 for V13; R12 adds V9 for V14; and R13 adds LocalQueryResultV10 for V15 callable, signature, compiler-symbol, and join subjects, while V1-V9 bytes and authority remain unchanged. | CLI black-box scenarios return the reviewed typed result and stable exit status; V15 exercises every R13 subject after restart, V14 exercises R12, V13 R11, V12 R10, V11 K1, V10 all seven V5 kinds, earlier heads retain approved dispatch/bytes, and portable family digests prove exact preservation. `E2E`, `CONF` |
...
| `FR-CLI-001` | P0 | `0.1` | The CLI MUST provide local `scan`, `docs`, `query`, `export`, and `explore` journeys with human-readable and versioned JSON output. R8, K1, and R10-R13 behavior MUST be explicitly selected, preserve accepted command bytes, use marker-owned atomic output roots, and reject unsupported profile composition before acquisition. | The R8 through R13 fixtures each complete scan -> docs -> query -> export -> explore without network, child process, browser auto-launch, target/toolchain/index execution, repository mutation, or unintended store mutation; complete invalid R13 composition returns ErrorV20, incomplete selector intent retains its historical error family, and every prior command remains unchanged. `E2E`, `CONF`, `SEC`, `FT` |
