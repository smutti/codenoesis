...
| `RW6` | Deterministic Cargo member-pattern compatibility | Proposed in issue #217: support only final one-component `prefix/*` declarations by projecting committed package manifests from immutable inventory. | Additive `FR-EXT-021` and Decision 0047; no Cargo execution, ambient filesystem discovery, general glob engine, dependency, or repository-specific rule. | Synthetic one-level/exclusion/overlap/capacity/determinism tests pass, literal-member outputs remain unchanged, and pinned wgpu advances beyond its current workspace rejection into the next honest progressive stage. |
