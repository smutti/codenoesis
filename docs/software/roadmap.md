...
| `RW7` | Cargo workspace package edition compatibility | Implemented by merged PR #220: allow the R3 planner to validate exact `edition.workspace = true` references from a committed non-empty root `[workspace.package].edition` string. | Approved `FR-EXT-022` and Decision 0048; no effective metadata materialization, Cargo execution, dependency resolution, schema, ontology, golden, new dependency, or repository-specific rule. | Synthetic virtual/non-virtual and malformed-reference tests pass with unchanged R3/R4 regressions. The complete corpus matches 8/8 reviewed oracles; pinned wgpu advances through package planning and stops at the next typed source-extraction boundary. |
| `RW8` | Bounded Rust parser and Cargo dotted dependency compatibility | Candidate implemented in issue #221: preserve offsets while deferring two exact tree-sitter gaps, reuse the clean positional tree downstream, and group legal dotted Cargo dependency fields into existing R4 declarations. | Additive `FR-EXT-023`, `FR-EXT-024`, and Decision 0049; no grammar upgrade, macro expansion, active-cfg selection, Cargo/rustc execution, schema, identity, new dependency, or repository-specific rule. | The Rust adapter suite and CLI R3 journey pass; the complete corpus matches 8/8 reviewed oracles. Pinned wgpu completes R3 with 2,604 entities and R4 with 3,400 entities, then stops deterministically at the typed R5 `rust.module trace` cfg-alternative conflict. |
...
10. Merged PR #220 closes Cargo workspace package edition inheritance and
    advances pinned wgpu into source extraction.
11. RW8 closes the bounded parser and dotted-dependency gaps, advancing wgpu
    through R4; the next breadth-first target is direct cfg module alternatives
    at R5, followed by the remaining manifest, flow, and acquisition gaps.
...
The first P1 Kotlin/KMP vertical merged in PR #225 (FR-EXT-025 / Decision 0051): committed declaration ontology, conventional module/source-set
membership, explicit expect/actual candidates and offline projections. See
[kotlin-kmp-v01.md](kotlin-kmp-v01.md) for its exact boundaries. The next branch
candidate is Java static declarations (FR-EXT-026 / Decision 0052), described in
[java-v01.md](java-v01.md). Compiler resolution and cross-language reconciliation
remain later work; the remaining language roadmap is unchanged.
