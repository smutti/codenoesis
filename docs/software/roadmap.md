...
| `G1` | Distribution and configuration | Proposed issue #182 G1a candidate: add a closed deterministic local CLI configuration and one unsigned digest-named staged-directory bundle with explicit clean-install, side-by-side upgrade, rollback, and uninstall procedures. Server artifacts, non-empty secret references, installers, release channels, signing, publication, support, and GA remain future G1/G5/G8/G9 work. | Proposed `FR-CFG-001`, `FR-REL-002`, `FR-CLI-008`; bounded `FR-CLI-002`, `NFR-CMP-001`, `NFR-SEC-003`, `NFR-PORT-001`, Decision 0034, `S14` | Embedded/explicit validation, invalid/unstable configuration, deterministic packaging, failure atomicity, actual-binary smoke, clean install, two-generation upgrade/rollback, and uninstall pass on every accepted experimental target without hidden state or secret leakage. |
