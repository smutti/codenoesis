...
| `B1` | Real-world Rust stability benchmark | Proposed branch-scoped candidate: activate one observational same-host suite over pinned Lekton success and RustDesk typed fail-closed behavior. B1a adds the exact 75-second benchmark-only selector; B1b corrects the bootstrap baseline and repository-identity-bound Lekton hashes after retained real evidence. | Issues #205/#206/#207, Decisions 0042/0043/0044, first selector-supporting baseline `cce84869430ef129f55591998b30ea2ea728e1c3`, no dependency or post-B1a Rust product change. | Separate three-sample baseline/candidate reports retain every sample, exact corrected semantic/outcome identity, 60-second selector-absent compatibility, exact 75-second benchmark boundary, reviewed p95 policy, privacy/no-network/no-target-execution controls, and no SLO or cross-host claim. |
