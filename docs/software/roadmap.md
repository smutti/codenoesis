...
| `RW1` | Explorable real-world Rust pilots | Implemented in the current package: one local runner generates snapshot, deterministic documentation, PortableGraph, and offline explorer for pinned Lekton at R16 and pinned RustDesk at boundary-safe R12. Generic fixes retain homogeneous direct-`cfg` value alternatives as unresolved evidence, split only oversized module documentation at statement boundaries under the unchanged 1 MiB limit, and admit validated gitlink references in PortableGraphV5 documentation. | Existing `FR-DOC-001`, `FR-EXP-004`, R12/R16 contracts, and the active B1 corpus identities; no new dependency or repository-specific ontology rule. | Focused synthetic regressions pass; the exact public repositories complete the combined runner with Lekton 26,158 entities and RustDesk 9,183 entities, and both produce loadable `index.html` explorers. |
| `RW2` | RustDesk R14-R16 boundary composition | Next: compose expression, local-flow, and safe-constant overlays with the already validated R12 gitlink boundary instead of selecting a lower operational profile. | Extend generic repository-boundary joins without reading or fetching nested source. | RustDesk completes the same R16 and FunctionContextV1 journey as Lekton while retaining the unbound `libs/hbb_common` boundary and deterministic replay. |
...
6. Complete RW1 so pinned Lekton and RustDesk both produce directly explorable
   ontology artifacts; keep RustDesk on the highest honest boundary-safe
   profile until RW2 closes R14-R16 boundary composition.
7. Build the LLM-ready projection over verified callable, evidence, source, and
   semantic-impact facts without promoting model output to product evidence.
8. Execute the reproducible conference evaluation over pinned, structurally
